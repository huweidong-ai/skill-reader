...
关键在它的加载机制，叫**渐进式加载**：平时只有每条 skill 的 description（一段触发场景 + 触发词）常驻在 Agent 的上下文里，成本极低；当你说了匹配的话——比如"帮我把这份数据做成 Excel 报表"——Agent 才把对应的 SKILL.md 正文加载进来，需要细节时再去读 references。
